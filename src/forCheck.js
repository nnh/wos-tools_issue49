...
function getCheckTargetFolderId_(){
  ...
  while (folders.hasNext()){
    ...
    lastUpdate.push([folder.getId(), folder.getLastUpdated(), folder.getName()]);
  }
...
function forCheck2(){
  ...
  while (inputFiles.hasNext()){
    ...
    const uids = [...new Set([json.papers.map(x => Number(x.uid.replace('WOS:', '')))])][0].sort((x, y) => x - y);
  ...
  while (outputFiles.hasNext()){
    ...
    const uids = [...new Set([values.map(x => Number(x[3].replace('WOS:', '')))])][0].sort((x, y) => x - y);
...
function forCheck1(){
  ...
  // 0件なら全て処理されている
  const res = inputFilenames.map(input => !outputFilenames.includes(input)? input : null).filter(x => x);  
  console.log(res);
}
